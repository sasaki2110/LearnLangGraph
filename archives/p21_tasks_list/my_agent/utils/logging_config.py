...
Morizo-aiv2のロギング実装を参考にした、シンプルなロギング設定
"""
...
class LoggingConfig:
    ...
    def __init__(self):
        # 環境変数からログファイルパスを取得
        log_file = os.getenv('LOG_FILE', 'p21_tasks_list.log')
    ...
    def setup_logging(self, log_level: str = "INFO", initialize: bool = True) -> logging.Logger:
        ...
        """
        # ルートロガーを作成
        root_logger = logging.getLogger('p21_tasks_list')
...
def get_logger(name: str) -> logging.Logger:
    ...
    Returns:
        ロガーインスタンス
    """
    return logging.getLogger(f'p21_tasks_list.{name}')
