# p33_ui_simple - UI確認用のストリーミング対応LangGraphエージェント

このプロジェクトは、UIでのストリーミング確認用のシンプルなLangGraphエージェントです。
Vercel AI SDKのチャットから呼び出せるストリーミング対応のエージェントです。
...
1. **nodeA**: あ行（あ、い、う、え、お）で始まる言葉を使った面白いジョークを生成し、jokesに追加します
2. **nodeK**: か行（か、き、く、け、こ）で始まる言葉を使った面白いジョークを生成し、jokesに追加します
3. **nodeS**: さ行（さ、し、す、せ、そ）で始まる言葉を使った面白いジョークを生成し、jokesに追加します
4. **node_final**: 生成されたジョーク集をまとめて、最終的な面白いジョークを作成します

## グラフ構造

```
start → nodeA → nodeK → nodeS → node_final → end
```

## 状態

- `messages`: メッセージ履歴（Vercel AI SDKからの入力と出力）
- `jokes`: ジョーク集（あ行・か行・さ行のジョークが順次追加される）
...
```bash
cd /root/LearnLangGraph/archives/p33_ui_simple
langgraph dev
```
...
- **通常ログ**: `p33_ui_simple.log` (デフォルト)
- **エラーログ**: `p33_ui_simple_error.log` (ERROR/CRITICALレベルのみ)
...
- `LOG_FILE`: ログファイル名 - デフォルト: `p33_ui_simple.log`
...
- **nodeA**: あ行のジョーク生成開始/完了
- **nodeK**: か行のジョーク生成開始/完了
- **nodeS**: さ行のジョーク生成開始/完了
- **node_final**: 最終ジョーク生成開始/完了
- **エラー**: エラー発生時の詳細情報

### ログの確認方法

1. **コンソール出力**: 実行時にコンソールにログが表示されます
2. **ログファイル**: `p33_ui_simple.log` ファイルを確認
3. **エラーログ**: `p33_ui_simple_error.log` ファイルでエラーのみを確認
...
- **ロギング機能**: p31_streamingを参考にした日本語ログを追加
