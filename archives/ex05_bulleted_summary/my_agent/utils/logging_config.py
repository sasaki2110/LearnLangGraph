...
p30_loggingとp31_streamingのロギング実装を参考にした、シンプルなロギング設定
"""
...
class LoggingConfig:
    ...
    def __init__(self):
        # 環境変数からログファイルパスを取得
        log_file = os.getenv('LOG_FILE', 'ex05_bulleted_summary.log')
    ...
    def setup_logging(self, log_level: str = "INFO", initialize: bool = True) -> logging.Logger:
        ...
        """
        # ルートロガーを作成
        root_logger = logging.getLogger('ex05_bulleted_summary')
...
def get_logger(name: str) -> logging.Logger:
    ...
    Returns:
        ロガーインスタンス
    """
    return logging.getLogger(f'ex05_bulleted_summary.{name}')
